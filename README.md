...
2. `追加` で結果セットを追加します。
3. 画面上部の `結果セット名` で名前を変更します。
4. `+ 行` / `- 行` で行を増減します。
5. `+ 列` / `- 列` で列を増減します。
6. グリッドのセルに値を入力します。
7. グリッド上部の `列名` で選択列の名前を変更します。
8. グリッド上部の `型` で選択セルの値型を指定します。
9. `プレビュー` で C# 出力内容を確認します。
10. `コピー` でプレビュー内容をクリップボードへコピーします。

行をドラッグすると表示順を変更できます。列はヘッダーをドラッグして並び替えできます。`↑ 行` / `↓ 行`、`← 列` / `→ 列` でも順序を変更できます。
