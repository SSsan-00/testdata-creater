...
2. `追加` で結果セットを追加します。
3. グリッド左上の固定操作エリアにある `+ 行` で行を追加します。
4. 各行の左端にある `- 行` で行を削除します。
5. グリッド左上の固定操作エリアにある `+ 列` で列を追加します。
6. グリッド最上段の各列にある `- 列` で列を削除します。
7. グリッドのセルに値を入力します。
8. 列ヘッダーをクリックしてフィールド名を直接変更します。
9. 画面上部の `列名` でも選択列の名前を変更できます。
10. 画面上部の `型` で選択セルの値型を指定します。型名は C# に近い表記で表示されます。
11. 下部の `プレビュー` で C# 出力内容を確認します。グリッドとプレビューの境界を上下にドラッグすると表示領域を変更できます。
12. `コピー` でプレビュー内容をクリップボードへコピーします。
